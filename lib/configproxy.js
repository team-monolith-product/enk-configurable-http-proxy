...
import { VertexTokenSource, decodeServiceAccountKey, defaultVertexApiUrl } from "./vertexauth.js";
...
function buildVertexProxy(options, log) {
  ...
  // 키가 없거나 깨졌어도 프록시 자체는 살려둔다. 여기서 throw 하면 전체 사용자 라우팅을
  // 담당하는 팟이 crashloop 에 빠진다. 다른 provider 와 같이 요청 시점에 500 으로 알린다.
...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    // Vertex 만 정적 키가 아니라 getAuthValue 로 요청 시점에 토큰을 받는다.
    this.apiProxies = [
      {
        name: "Anthropic",
      ...
      buildVertexProxy(options, this.log),
  ...
  async handleApiProxy(req, res, cfg) {
    ...
    if (!cfg.apiKey && !cfg.getAuthValue) {
    ...
    // 토큰 발급은 IP 검사를 통과한 뒤에 한다. 미등록 IP 가 토큰 발급을 유발하지 못하게 막는다.
    var authValue = cfg.apiKey;
    if (cfg.getAuthValue) {
      try {
        authValue = await cfg.getAuthValue();
      } catch (e) {
        this.log.error("%s proxy: access token issuance failed: %s", cfg.name, e.message);
        sentry.captureApiProxyError(cfg.name, e, req.method, _logUrl(req.url));
        fail(req, res, 502, cfg.name + " access token issuance failed");
        return true;
      }
    }
