...
// 끝의 슬래시를 모두 제거한다. /\/+$/ 정규식은 백트래킹으로 polynomial ReDoS
// 경고를 유발하므로(CodeQL js/polynomial-redos) 선형 스캔으로 대체한다.
...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    this.anthropicProxyPath = options.anthropicProxyPath
      ? stripTrailingSlashes(options.anthropicProxyPath)
      : null;
    this.anthropicApiUrl = options.anthropicApiUrl || "https://api.anthropic.com";
    this.anthropicApiKey = options.anthropicApiKey;

    // Gemini API proxy: 동일 패턴. singleuser pod 에서는 key 없이 호출하고
    // CHP 가 x-goog-api-key 헤더에 실키를 주입한다.
    this.geminiProxyPath = options.geminiProxyPath
      ? stripTrailingSlashes(options.geminiProxyPath)
      : null;
    this.geminiApiUrl = options.geminiApiUrl || "https://generativelanguage.googleapis.com";
    this.geminiApiKey = options.geminiApiKey;

    // DeepSeek API proxy: DeepSeek 는 Anthropic 호환 엔드포인트를 제공하므로 anthropic 과
    // 동일하게 x-api-key 헤더에 실키를 주입한다. 타깃 base 에 /anthropic 경로가 포함된다.
    this.deepseekProxyPath = options.deepseekProxyPath
      ? stripTrailingSlashes(options.deepseekProxyPath)
      : null;
    this.deepseekApiUrl = options.deepseekApiUrl || "https://api.deepseek.com/anthropic";
  ...
  async handleGeminiProxy(req, res) {
    // Proxy requests to the Gemini API with key injection. anthropic 경로와 동일하게
    // singleuser pod IP 검증을 통과한 요청에만 x-goog-api-key 헤더로 실키를 주입한다.
    if (!this.geminiProxyPath) return false;

    var urlPath = decodeURIComponent(new URL(req.url, "http://example.com").pathname);
    if (!urlPath.startsWith(this.geminiProxyPath + "/") && urlPath !== this.geminiProxyPath) {
      return false;
    }

    if (!this.geminiApiKey) {
      this.log.error("Gemini proxy: GEMINI_API_KEY not configured");
      fail(req, res, 500, "Gemini proxy not configured");
      return true;
    }
    ...
    if (!route) {
      this.log.warn(
        "Gemini proxy: rejected request from unregistered IP %s",
        remoteAddress
      );
    ...
    this.log.debug("Gemini proxy: request from %s (%s)", route, remoteAddress);

    var strippedUrl = req.url.slice(this.geminiProxyPath.length);
    if (!strippedUrl.startsWith("/")) strippedUrl = "/" + strippedUrl;
    req.url = strippedUrl;

    // x-goog-api-key 헤더에 실키 주입. dummy 가 외부로 전달되지 않도록 무조건 덮어쓴다.
    req.headers["x-goog-api-key"] = this.geminiApiKey;

    delete req.headers["host"];

    var target = new URL(this.geminiApiUrl);
    ...
    this.proxy.web(req, res, proxyOptions, function (e) {
      that.log.error("Gemini proxy error: %s", e.message);
      if (!res.headersSent) {
        fail(req, res, 502, "Gemini API request failed");
      }
    });
  ...
  async handleDeepseekProxy(req, res) {
    // Proxy requests to the DeepSeek API with key injection. anthropic 경로와 동일한
    // network 기반 인증(등록된 singleuser pod IP 검증)을 적용한다. DeepSeek 는 Anthropic
    // 호환 엔드포인트(.../anthropic)를 제공하므로 anthropic 과 같은 x-api-key 를 주입한다.
    if (!this.deepseekProxyPath) return false;

    var urlPath = decodeURIComponent(new URL(req.url, "http://example.com").pathname);
    if (!urlPath.startsWith(this.deepseekProxyPath + "/") && urlPath !== this.deepseekProxyPath) {
      return false;
    }

    if (!this.deepseekApiKey) {
      this.log.error("DeepSeek proxy: DEEPSEEK_API_KEY not configured");
      fail(req, res, 500, "DeepSeek proxy not configured");
      return true;
    }
    ...
    if (!route) {
      this.log.warn(
        "DeepSeek proxy: rejected request from unregistered IP %s",
        remoteAddress
      );
      fail(req, res, 403, "Forbidden");
      return true;
    ...
    this.log.debug("DeepSeek proxy: request from %s (%s)", route, remoteAddress);

    var strippedUrl = req.url.slice(this.deepseekProxyPath.length);
    if (!strippedUrl.startsWith("/")) strippedUrl = "/" + strippedUrl;
    req.url = strippedUrl;

    // x-api-key 헤더에 실키 주입. dummy 가 외부로 전달되지 않도록 무조건 덮어쓴다.
    req.headers["x-api-key"] = this.deepseekApiKey;

    delete req.headers["host"];

    var target = new URL(this.deepseekApiUrl);
    var isHttps = target.protocol.startsWith("https");
    var proxyOptions = {
      target: target,
      changeOrigin: true,
    ...
    var that = this;
    this.proxy.web(req, res, proxyOptions, function (e) {
      that.log.error("DeepSeek proxy error: %s", e.message);
      if (!res.headersSent) {
        fail(req, res, 502, "DeepSeek API request failed");
      }
    });
  ...
  async handleProxyWeb(req, res) {
    ...
    // Gemini proxy intercepts before normal routing
    handled = await this.handleGeminiProxy(req, res);
    if (handled) return;

    // DeepSeek proxy intercepts before normal routing
    handled = await this.handleDeepseekProxy(req, res);
    if (handled) return;
