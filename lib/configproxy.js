...
import { LlmHealthChecker } from "./llm_health_check.js";
...
function _decodeBody(buffer, encoding) {
  try {
    ...
    return "<" + encoding + " body: decode failed>";
  }
  return buffer.toString("utf8").slice(0, 4096);
}
...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    this.tutorialServePort = options.tutorialServePort || 3001;
    ...
    // DeepSeek 와 MiniMax 는 Anthropic 호환 엔드포인트(.../anthropic)라 anthropic 과 같은 x-api-key 를 쓴다.
    // providerID 는 opencode(models.dev) 표기 — /llm_health_check 응답 키와 메트릭 라벨에 쓴다.
    // probeModel 기본값은 opencode 가 실제 쓰는 모델(rails Hackathon::AI_MODELS 화이트리스트,
    // opencode 기본 폴백 풀)과 일치시켜 응답 키가 폴백 풀 엔트리와 문자열 그대로 매칭되게 한다.
    this.apiProxies = [
      {
        name: "Anthropic",
        ...
        probeKind: "anthropic",
        probeModel: options.anthropicProbeModel || "claude-haiku-4-5",
      },
      {
        name: "Gemini",
        providerID: "google",
        ...
        probeKind: "gemini",
        probeModel: options.geminiProbeModel || "gemini-3.5-flash",
      },
      {
        name: "DeepSeek",
        providerID: "deepseek",
        ...
        probeKind: "anthropic",
        probeModel: options.deepseekProbeModel || "deepseek-v4-pro",
      },
      {
        name: "MiniMax",
        providerID: "minimax",
        proxyPath: options.minimaxProxyPath ? trimPrefix(options.minimaxProxyPath) : null,
        apiUrl: options.minimaxApiUrl || "https://api.minimax.io/anthropic",
        apiKey: options.minimaxApiKey,
        header: "x-api-key",
        envName: "MINIMAX_API_KEY",
        probeKind: "anthropic",
        probeModel: options.minimaxProbeModel || "MiniMax-M3",
      },
    ...
    if (this.apiProxies.length) {
      this.llmHealth = new LlmHealthChecker({
        log: this.log,
        metrics: this.metrics,
        providers: this.apiProxies,
        ttl: options.llmHealthTtl,
        probeTimeout: options.llmHealthProbeTimeout,
        rateLimitTtl: options.llmHealthRateLimitTtl,
        valkeyUrl: options.valkeyUrl,
        valkeyKeyPrefix: options.valkeyKeyPrefix,
        valkeyAuthToken: options.valkeyAuthToken,
        valkeyConfig: options.valkeyConfig,
      });
    }
    ...
    this.proxy.on("proxyRes", function (proxyRes, req, res) {
      ...
      if (req._apiProxyProviderID && proxyRes.statusCode === 429 && that.llmHealth) {
        that.llmHealth.noteTrafficRateLimit(req._apiProxyProviderID, proxyRes.headers);
      }
      if (req._apiProxyName && proxyRes.statusCode >= 400) {
        var chunks = [];
        var size = 0;
        ...
        proxyRes.on("end", function () {
          var body = _decodeBody(Buffer.concat(chunks), proxyRes.headers["content-encoding"]);
          that.log.error(
            "%s API upstream error: %s %s %s %s",
            req._apiProxyName,
            proxyRes.statusCode,
            req.method,
            _logUrl(req.url),
          ...
          sentry.captureApiProxyResponse(
            req._apiProxyName,
            proxyRes.statusCode,
            req.method,
            _logUrl(req.url),
  ...
  async targetForServeReq(req) {
    ...
    // {username}-tutorial.{serveDomain} 은 같은 pod 의 튜토리얼 dev 서버로 라우팅한다.
    // opencodeId 는 소문자+숫자 16자(하이픈 없음)라 실제 username 과 충돌하지 않는다.
    var targetPort = this.servePort;
    var tutorialSuffix = "-tutorial";
    if (username.endsWith(tutorialSuffix)) {
      username = username.slice(0, -tutorialSuffix.length);
      if (!username) return undefined;
      targetPort = this.tutorialServePort;
    }
  ...
  async handleApiProxy(req, res, cfg) {
    ...
    req.url = strippedUrl;
    req._apiProxyName = cfg.name;
    req._apiProxyProviderID = cfg.providerID;
    ...
    this.proxy.web(req, res, proxyOptions, function (e) {
      ...
      sentry.captureApiProxyError(cfg.name, e, req.method, _logUrl(req.url));
      if (!res.headersSent) {
  ...
  async handleProxyWeb(req, res) {
    ...
    if (await this.handleLlmHealthCheck(req, res)) return;
  ...
  // opencode(singleuser pod)가 폴링하는 LLM 프로바이더 헬스 엔드포인트.
  // API 프록시와 동일하게 등록된 pod IP 만 허용한다 — proxy-public 은 인터넷에
  // 노출되어 있고, 폴링이 프로브(실 API 호출)를 트리거할 수 있기 때문.
  async handleLlmHealthCheck(req, res) {
    if (!this.llmHealth) return false;
    var urlPath = new URL(req.url, "http://example.com").pathname;
    if (urlPath !== "/llm_health_check") return false;
    ...
    var route = await this._isSingleuserPod(remoteAddress);
    if (!route) {
      this.log.warn("llm_health_check: rejected request from unregistered IP %s", remoteAddress);
      fail(req, res, 403, "Forbidden");
      return true;
    }

    try {
      var result = await this.llmHealth.check();
      res.writeHead(200, { "Content-Type": "application/json" });
      res.write(JSON.stringify(result));
      res.end();
    } catch (e) {
      this.log.error("llm_health_check failed: %s", e.message);
      fail(req, res, 503, "health state unavailable");
    }
    return true;
  }
