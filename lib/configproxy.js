...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    // Upstage 는 OpenAI 호환 엔드포인트라 authorization 헤더에 Bearer 접두(headerPrefix)를 붙여 주입한다.
    this.apiProxies = [
      {
        name: "Anthropic",
      ...
      {
        name: "Upstage",
        proxyPath: options.upstageProxyPath ? trimPrefix(options.upstageProxyPath) : null,
        apiUrl: options.upstageApiUrl || "https://api.upstage.ai",
        apiKey: options.upstageApiKey,
        header: "authorization",
        headerPrefix: "Bearer ",
        envName: "UPSTAGE_API_KEY",
      },
  ...
  async handleApiProxy(req, res, cfg) {
    ...
    req.headers[cfg.header] = (cfg.headerPrefix || "") + cfg.apiKey;
