...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    this.servePath = options.servePath ? options.servePath.replace(/\/+$/, "") : null;
  ...
  async targetForServeReq(req) {
    // resolve serve path requests to user pod targets on a different port
    if (!this.servePath) return undefined;

    var urlPath = decodeURIComponent(new URL(req.url, "http://example.com").pathname);
    var servePrefix = this.servePath;

    // match /serve/username or /serve/username/...
    if (!urlPath.startsWith(servePrefix + "/") && urlPath !== servePrefix) {
      // Fallback: bare asset paths like /_next/static/... that Next.js emits
      // without the serve prefix. Resolve the user from the Referer header.
      return this._resolveServeAssetFallback(req, urlPath);
    }

    var afterPrefix = urlPath.slice(servePrefix.length);
    var parts = afterPrefix.split("/").filter(Boolean);
  ...
  // Framework dev servers (Next.js, Vite, etc.) emit asset references as
  // absolute paths (e.g. /_next/static/..., /@vite/client, /src/main.tsx)
  // without the /serve/user prefix. When the Referer header indicates the
  // request originated from a /serve/ preview page, we route it to the
  // corresponding user's dev server. The Referer check itself limits the
  // scope — only requests initiated from a preview iframe are affected.
  async _resolveServeAssetFallback(req, urlPath) {
    // Skip paths that belong to known system routes — these must never be
    // hijacked even if the Referer happens to contain /serve/.
    // /user/: singleuser pod routes registered in the routing table
    // /hub/:  JupyterHub auth/login pages
    if (
      urlPath.startsWith("/user/") ||
      urlPath.startsWith("/hub/")
    ) return undefined;
    ...
    try {
      ...
      var refPath = decodeURIComponent(refUrl.pathname);
    } catch (e) {
      return undefined;
    }

    var servePrefix = this.servePath;
    if (!refPath.startsWith(servePrefix + "/")) return undefined;

    var afterPrefix = refPath.slice(servePrefix.length);
    var parts = afterPrefix.split("/").filter(Boolean);
    if (parts.length === 0) return undefined;
    ...
    this.log.debug("SERVE ASSET FALLBACK %s → %s (user: %s)", urlPath, targetUrl.toString(), username);

    // prefix is empty string so the full req.url (e.g. /_next/static/...,
    // /@vite/client) is forwarded as-is to the upstream dev server
    return {
      prefix: "",
      target: targetUrl.toString(),
      _isServe: true,
    ...
    return this.targetForServeReq(req)
      ...
      .then((match) => {
        ...
        if (match._isServe) {
          ...
          // Rewrite Host header so upstream dev servers see localhost instead
          // of the external domain. Without this, Vite 6.x (CVE-2025-24010)
          // rejects the request because the original host (e.g.
          // opencode.dev.team-mono.com) is not in server.allowedHosts.
          req.headers.host = "localhost:" + that.servePort;
