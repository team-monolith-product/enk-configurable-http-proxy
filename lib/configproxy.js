...
export class ConfigurableProxy extends EventEmitter {
  ...
  // Next.js (and similar frameworks) emit asset references as absolute paths
  // like /_next/static/..., which the browser requests without the /serve/user
  // prefix. We inspect the Referer header to determine which user's dev server
  // should handle the request and proxy it there with an empty prefix so the
  // full original URL is forwarded unchanged.
  async _resolveServeAssetFallback(req, urlPath) {
    // Only handle paths that look like framework assets
    if (!urlPath.startsWith("/_next/")) return undefined;
    ...
    // prefix is empty string so the full req.url (e.g. /_next/static/...) is
    // forwarded as-is to the upstream Next.js dev server
    return {
      prefix: "",
      target: targetUrl.toString(),
      _isServe: true,
