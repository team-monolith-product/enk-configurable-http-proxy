...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    this.anthropicHubApiUrl = options.anthropicHubApiUrl || "http://hub:8081/hub/api";
    // Cache validated tokens for 60s to avoid hitting Hub API on every request
    this._anthropicTokenCache = new Map();
  ...
  _validateHubToken(token) {
    // Validate a JupyterHub API token by calling the Hub API.
    // Returns a promise resolving to user info on success, null on failure.
    // Results are cached for 60 seconds.
    var cached = this._anthropicTokenCache.get(token);
    if (cached && cached.expires > Date.now()) {
      return Promise.resolve(cached.user);
    }

    var that = this;
    var url = new URL(this.anthropicHubApiUrl + "/authorizations/token/" + token);
    ...
    return new Promise(function (resolve) {
      var req = mod.request(
        ...
        {
          method: "GET",
          headers: { Authorization: "token " + (that.authToken || "") },
        },
        function (res) {
          var body = "";
          ...
          res.on("end", function () {
            if (res.statusCode === 200) {
              try {
                ...
                that._anthropicTokenCache.set(token, {
                  user: user,
                  expires: Date.now() + 60000,
                });
                resolve(user);
              } catch (e) {
                that.log.error("Hub token validation: invalid JSON response");
                resolve(null);
              }
            } else {
              resolve(null);
      ...
      req.on("error", function (e) {
        that.log.error("Hub token validation failed: %s", e.message);
        resolve(null);
      });
      req.end();
    });
  ...
  async handleAnthropicProxy(req, res) {
    ...
    // The singleuser pod sends its JUPYTERHUB_API_TOKEN in the x-api-key header.
    // This handler validates the token against the Hub API, replaces the header
    // with the real ANTHROPIC_API_KEY, and forwards to api.anthropic.com.
    ...
    // Extract the token from x-api-key header
    var incomingToken = req.headers["x-api-key"];
    if (!incomingToken) {
      this.log.warn("Anthropic proxy: missing x-api-key header from %s", req.socket.remoteAddress);
      fail(req, res, 401, "Missing authentication token");
      return true;
    }

    // Validate against JupyterHub API
    var user = await this._validateHubToken(incomingToken);
    if (!user) {
      this.log.warn("Anthropic proxy: invalid token from %s", req.socket.remoteAddress);
      fail(req, res, 403, "Invalid authentication token");
      return true;
    }

    this.log.debug(
      "Anthropic proxy: authenticated request from user %s",
      user.name || "unknown"
    );
