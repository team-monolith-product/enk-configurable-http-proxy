...
import { createVertexProxy } from "./vertexauth.js";
...
// 정적 키 provider 와 Vertex 를 같은 계약으로 맞춘다. 요청 경로가 getAuthValue 하나만
// 보면 되도록 여기서 한 번 정규화한다. 키가 없으면 null 로 남아 요청 시점에 500 이 된다.
function withAuthResolver(cfg) {
  if (cfg.getAuthValue) return cfg;
  return { ...cfg, getAuthValue: cfg.apiKey ? () => cfg.apiKey : null };
}
...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    // OpenAI 는 Authorization: Bearer 규약이라 헤더 값에 접두가 붙는다.
    // Vertex 는 정적 키가 없어 별도 모듈에서 항목을 만든다.
    this.apiProxies = [
      {
        name: "Anthropic",
      ...
      {
        name: "OpenAI",
        proxyPath: options.openaiProxyPath ? trimPrefix(options.openaiProxyPath) : null,
        apiUrl: options.openaiApiUrl || "https://api.openai.com",
        apiKey: options.openaiApiKey ? "Bearer " + options.openaiApiKey : null,
        header: "authorization",
        envName: "OPENAI_API_KEY",
      },
      createVertexProxy(options, this.log),
    ]
      .filter((p) => p && p.proxyPath)
      .map(withAuthResolver);
  ...
  async handleApiProxy(req, res, cfg) {
    ...
    // Vertex 는 여기서 토큰을 발급한다. IP 검사 뒤에 두어 미등록 IP 가 발급을 유발하지 못하게 막는다.
    var authValue;
    try {
      authValue = await cfg.getAuthValue();
    } catch (e) {
      this.log.error("%s proxy: credential resolution failed: %s", cfg.name, e.message);
      sentry.captureApiProxyError(cfg.name, e, req.method, _logUrl(req.url));
      fail(req, res, 502, cfg.name + " credential resolution failed");
      return true;
    }
    ...
    req.headers[cfg.header] = authValue;
