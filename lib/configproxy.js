...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    // DeepSeek, MiniMax, Kimi 는 Anthropic 호환 엔드포인트(.../anthropic)라 anthropic 과 같은 x-api-key 를 쓴다.
    this.apiProxies = [
      {
        name: "Anthropic",
      ...
      {
        name: "Kimi",
        proxyPath: options.kimiProxyPath ? trimPrefix(options.kimiProxyPath) : null,
        apiUrl: options.kimiApiUrl || "https://api.moonshot.ai/anthropic",
  ...
  async handleApiProxy(req, res, cfg) {
    ...
    // host 삭제 후 xfwd 가 빈 x-forwarded-host 를 붙이면 Moonshot 이 이를 Host 로 채택해
    // 400 (missing required Host header) 을 반환한다. 외부 API 로는 x-forwarded-* 를 보내지 않는다.
    for (var headerName of Object.keys(req.headers)) {
      if (headerName.startsWith("x-forwarded-")) delete req.headers[headerName];
    }
